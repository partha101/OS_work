...
// Lab3
// clone a parent thread same as fork()
// address space is shared
// stack start address by the caller function is used for space allocation
int clone(void* stack) {
  ...
  struct proc *np;
  struct proc *p = myproc();
  int pg_size = PGSIZE*sizeof(void);
...
if(map_result < 0){
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  np->trapframe->sp = (uint64)(stack + pg_size);
